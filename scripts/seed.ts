  ...
  await db.transaction(async (tx) => {
    ...
    if (initialAdminEmail) {
      const [adminAccount] = await tx
        ...
        .where(
          and(
            sql`lower(trim(${user.email})) = ${initialAdminEmail}`,
            sql`${user.deletedAt} is null`,
          ),
        )
        .limit(1)
      if (adminAccount) {
        const result = await ensureInitialAdminTx(tx, adminAccount, process.env, 'seed')
        if (!result && !adminAccount.emailVerified) {
          console.warn('Seed skipped INITIAL_ADMIN_EMAIL because the matching account is not verified.')
        }
      }
    }
