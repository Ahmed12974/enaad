...
try {
  ...
  await applyMigration('0013-backup-jobs', ['drizzle/0013_backup_jobs.sql'])
  await applyMigration('0014-section-metadata', ['drizzle/0014_section_metadata.sql'])
  await applyMigration('0015-competition-lifecycle', ['drizzle/0015_competition_lifecycle.sql'])
  await applyMigration('0016-challenge-type', ['drizzle/0016_challenge_type.sql'])
  await applyMigration('0017-branch-compatibility', ['drizzle/0017_branch_compatibility.sql'])
  ...
  const validation = await client.query(`select
    ...
    to_regclass('public."backupJobs"') is not null as "backupJobsTable",
    exists (select 1 from information_schema.columns where table_schema='public' and table_name='educationalSections' and column_name='isActive') as "sectionMetadataColumns",
    exists (
      select 1 from pg_constraint
      where conname = 'educational_sections_type_check'
        and pg_get_constraintdef(oid) like '%subject%'
        and pg_get_constraintdef(oid) like '%language%'
        and pg_get_constraintdef(oid) like '%skills%'
    ) as "canonicalSectionTypeConstraint",
    not exists (
      ...
      where table_schema = 'public'
        ...
        and is_nullable = 'NO'
    ) as "legacySectionNameOptional",
    exists (
      select 1 from information_schema.columns
      where table_schema = 'public'
        and table_name = 'educationalSections'
        and column_name = 'sectionType'
        and column_default like '%subject%'
    ) as "canonicalSectionTypeDefault",
    to_regclass('public."competitionMathSections"') is not null as "competitionMathSectionsTable",
    exists (select 1 from information_schema.columns where table_schema='public' and table_name='competitions' and column_name='lifecycle') as "competitionLifecycleColumns",
    exists (
      select 1 from pg_constraint
      where conname = 'challenge_settings_competition_type_check'
    ) as "challengeTypeConstraint",
    exists (
      select 1 from information_schema.columns
