...
import { addBulk, addSentence, addWord, deleteWord, toggleFavorite, updateWord } from '@/app/actions'
...
export function Studio({ view, data }: { view: string; data: StudioData }) {
  ...
  if (view === 'add') return <Add start={start} pending={pending} />
...
function Bank({
  ...
  const [q, setQ] = useState(''),
    ...
    [editing, setEditing] = useState<W | null>(null),
    [actionMessage, setActionMessage] = useState<{ ok: boolean; text: string } | null>(null),
    ...
    <main>
      ...
      <div className="toolbar">
        <div className="search">
          <Search />
          <Input aria-label="ابحث بالإنجليزية أو العربية..."
            value={q}
      ...
      {actionMessage && (
        <Alert variant={actionMessage.ok ? 'default' : 'destructive'}>
      ...
      {editing && !isSentences && (
        <Card className="word-edit-card">
          <CardHeader>
            <CardTitle>تعديل الكلمة</CardTitle>
            <CardDescription>احفظ التعديلات الفعلية في حسابك، ثم ستتحدث القائمة تلقائيًا.</CardDescription>
          </CardHeader>
          <CardContent>
            <form
              className="form-grid"
              onSubmit={(event) => {
                ...
                start(async () => {
                  try {
                    const result = await updateWord(editing.id, form)
                    setActionMessage({
                      ok: result.ok,
                      text: result.ok ? 'تم تعديل الكلمة وحفظها.' : result.message,
                    })
                    if (result.ok) setEditing(null)
                  } catch {
                    setActionMessage({ ok: false, text: 'تعذر تعديل الكلمة. حاول مرة أخرى.' })
                  }
                })
              }}
            >
              <label>
                الكلمة
                <Input name="word" defaultValue={editing.word} required maxLength={120} dir="ltr" />
              </label>
              <label>
                المعنى
                <Input name="meaning" defaultValue={editing.meaning} required maxLength={300} />
              </label>
              <label>
                اللغة
                <select name="language" defaultValue={editing.language ?? 'en'}>
              ...
              <label>
                التصنيف النصي
                <Input name="category" defaultValue={editing.category ?? 'عام'} maxLength={80} />
              </label>
              <label>
                المستوى
                <select name="level" defaultValue={editing.level ?? 'A1'}>
              ...
              <label>
                الصعوبة
                <select name="difficulty" defaultValue={editing.difficulty ?? 'easy'}>
              ...
              <div className="admin-row">
                <Button type="submit" disabled={pending}>حفظ التعديل</Button>
          ...
          {isSentences
            ...
            : (list as W[]).map((item) => (
                <div className="bank-row" key={item.id}>
                  ...
                  <button
                    type="button"
                    aria-label={`تشغيل نطق ${item.word}`}
                    disabled={pending}
                    onClick={() => {
                      if (!('speechSynthesis' in window)) {
                        setActionMessage({ ok: false, text: 'ميزة النطق غير مدعومة في هذا المتصفح.' })
                        return
                      }
                      window.speechSynthesis.cancel()
                      const utterance = new SpeechSynthesisUtterance(item.word)
                      utterance.lang = item.language === 'ar' ? 'ar' : 'en-US'
                    ...
                    onClick={() =>
                      start(async () => {
                        try {
                          ...
                          setActionMessage({ ok: true, text: 'تم تحديث المفضلة.' })
                        } catch {
                          setActionMessage({ ok: false, text: 'تعذر تحديث المفضلة.' })
                        }
                      })
                    }
                  >
        ...
        onConfirm={() => {
          ...
          start(async () => {
            try {
              ...
              setActionMessage({ ok: true, text: 'تم حذف الكلمة.' })
              setDeleteId(null)
            } catch {
              setActionMessage({ ok: false, text: 'تعذر حذف الكلمة.' })
            }
          })
        }}
      />
              ...
              onSubmit={(event) => {
                ...
                start(async () => {
                  try {
                    ...
                    setMessage({
                      ok: result.ok,
                      text: result.ok ? 'تم حفظ الكلمة في حسابك.' : result.message,
                    })
                    if (result.ok) formElement.reset()
                  } catch {
                    setMessage({ ok: false, text: 'تعذر حفظ الكلمة. حاول مرة أخرى.' })
                  }
                })
              }}
            >
              ...
              <Button type="submit" disabled={pending}>
                <Sparkles /> {pending ? 'جارٍ الحفظ…' : 'حفظ الكلمة'}
              </Button>
            </form>
          )}
          {tab === 'sentence' && (
            <form
              ...
              onSubmit={(event) => {
                ...
                start(async () => {
                  try {
                    const result = await addSentence(form)
                    setMessage({
                      ok: result.ok,
                      text: result.ok ? 'تم حفظ الجملة في حسابك.' : result.message,
                    })
                    if (result.ok) formElement.reset()
                  } catch {
                    setMessage({ ok: false, text: 'تعذر حفظ الجملة. حاول مرة أخرى.' })
                  }
                })
              }}
            >
              ...
              <Button type="submit" disabled={pending}>{pending ? 'جارٍ الحفظ…' : 'حفظ الجملة'}</Button>
            </form>
          )}
          {tab === 'bulk' && (
            <div className="form-stack">
              <Textarea aria-label="Protect - يحمي Challenge - تحدٍ"
                dir="ltr"
                rows={9}
                value={bulk}
              ...
              <Button
                ...
                onClick={() =>
                  start(async () => {
                    try {
                      const result = await addBulk(bulk)
                      setMessage({
                        ok: result.added > 0,
                        text: `تمت إضافة ${result.added} وتجاوز ${result.skipped} سطرًا.`,
                      })
                      if (result.added > 0) setBulk('')
                    } catch (error) {
                      setMessage({
                        ok: false,
                        text: publicActionError(error, 'تعذر إضافة الكلمات.'),
                      })
                    }
                  })
                }
              >
                {pending ? 'جارٍ الإضافة…' : 'إضافة الكل إلى البنك'}
              </Button>
            </div>
          )}
        </CardContent>
