...
function competitionScopeLabel(scope: string) {
  return (
    {
      en: 'اللغة الإنجليزية',
      ar: 'اللغة العربية',
  ...
  return (
    <main className="admin-console" dir="rtl">
      <header className="admin-console-header">
        ...
        <Badge>
          <ShieldCheck aria-hidden="true" /> مدير النظام
        </Badge>
      </header>
      ...
      <Card>
        ...
        <CardContent>
          <form className="admin-filter-form" method="get" action="/admin/users">
            <Input aria-label="الاسم أو البريد أو رقم المستخدم"
              name="search"
              ...
              <details className="admin-user-card">
                ...
                <div className="admin-user-details">
                  <dl>
                    ...
                    <div>
                      <dt>الصلاحية</dt>
                      <dd>{item.role === 'admin' ? 'مدير' : 'مستخدم'}</dd>
                    </div>
                    <div>
                  ...
                  <div className="admin-operation-grid">
                    ...
                    >
                      ...
                      <Input aria-label="سبب القرار" name="reason" required minLength={5} placeholder="سبب القرار" />
                    </ManagedForm>
                    <ManagedForm
                    ...
                    >
                      ...
                      <Input aria-label="سبب الترقية أو التخفيض" name="reason" required minLength={5} placeholder="سبب الترقية أو التخفيض" />
                    </ManagedForm>
                    <ManagedForm action={addUserAdminNote} submitLabel="إضافة ملاحظة">
                      ...
                      <Textarea aria-label="الملاحظة الإدارية" name="note" required minLength={3} placeholder="الملاحظة الإدارية" />
                    </ManagedForm>
                    <ManagedForm
        ...
        {data.sections.map((section) => (
          <Card key={section.id} style={{ borderTopColor: section.color ?? undefined }}>
            ...
            <CardContent className="admin-stack-sm">
              ...
              >
                <Input aria-label="سبب الأرشفة" name="reason" required minLength={5} placeholder="سبب الأرشفة" />
              </ManagedForm>
            </CardContent>
          </Card>
        ))}
          ...
          <CardContent className="admin-list">
            {data.archivedSections.map((item) => (
              <div className="admin-list-row" key={item.id}>
                ...
                >
                  <Input aria-label="سبب الاستعادة" name="reason" required minLength={5} placeholder="سبب الاستعادة" />
                </ManagedForm>
              </div>
            ))}
          </CardContent>
    ...
    <ManagedForm action={action} submitLabel={section ? 'حفظ التعديلات' : 'إضافة القسم'}>
      <div className="admin-form-grid">
        <label className="admin-field">
          ...
          <Input name="name" required minLength={2} defaultValue={section?.name} placeholder="مثل: القراءة" />
        </label>
        <label className="admin-field">
          <span>اسم القسم بالإنجليزية (اختياري)</span>
          <Input name="nameEn" dir="ltr" defaultValue={section?.nameEn ?? ''} placeholder="Reading" />
        </label>
        <label className="admin-field">
          <span>المعرّف داخل الرابط</span>
          <Input
            name="slug"
            required
            dir="ltr"
        ...
        <label className="admin-field">
          <span>نوع القسم</span>
          <select name="sectionType" defaultValue={section?.sectionType ?? 'subject'}>
            <option value="subject">مادة تعليمية</option>
        ...
        <label className="admin-field admin-field-wide">
          ...
          <Input
            ...
            placeholder="وصف يظهر في بطاقة القسم"
          />
        </label>
        <label className="admin-field">
          <span>لون القسم</span>
          <div className="admin-color-field">
            <Input name="color" type="color" defaultValue={section?.color ?? '#2563eb'} />
            <code>{section?.color ?? '#2563eb'}</code>
          </div>
        </label>
        <label className="admin-field">
          <span>الفئة العمرية</span>
          <Input name="ageRange" defaultValue={section?.ageRange ?? ''} placeholder="مثل: 8–12 سنة" />
        </label>
        <label className="admin-field">
          <span>المستوى المستهدف</span>
          <Input name="targetLevel" defaultValue={section?.targetLevel ?? ''} placeholder="مبتدئ أو متوسط" />
        </label>
        <label className="admin-field">
          <span>أيقونة القسم</span>
          <select name="iconMediaId" defaultValue={section?.iconMediaId ?? ''}>
        ...
        <label className="admin-field">
          ...
          <select name="imageMediaId" defaultValue={section?.imageMediaId ?? ''}>
        ...
        <label className="admin-field">
          <span>القسم السابق المطلوب</span>
          <select
            name="prerequisiteSectionId"
            defaultValue={section?.previousSectionId ?? stringFromRecord(unlockRules, 'prerequisiteSectionId')}
          >
        ...
        <label className="admin-field">
          ...
          <select name="access" defaultValue={section?.access ?? 'free'}>
            <option value="free">متاح للجميع</option>
            <option value="restricted">يتطلب شروط فتح</option>
          </select>
        </label>
        <label className="admin-field">
          <span>حالة النشر</span>
          <select name="status" defaultValue={section?.status ?? 'draft'}>
        ...
        <label className="admin-field">
          <span>الترتيب</span>
          <Input name="sortOrder" type="number" min="0" defaultValue={section?.sortOrder ?? 0} />
        </label>
        <label className="admin-check-field">
          <input name="isActive" type="checkbox" defaultChecked={section?.isActive ?? true} />
          <span>القسم مفعّل ويظهر عند نشره</span>
        </label>
        <label className="admin-check-field">
          <input name="isPaid" type="checkbox" defaultChecked={section?.isPaid ?? false} />
          <span>قسم مدفوع</span>
        </label>
        <label className="admin-field">
          <span>السعر بأصغر وحدة للعملة</span>
          <Input
            name="priceMinor"
            type="number"
            min="0"
            defaultValue={section?.priceMinor ?? 0}
        ...
        <label className="admin-field">
          ...
          <Input
            ...
            maxLength={3}
            pattern="[A-Za-z]{3}"
            defaultValue={section?.currency ?? 'USD'}
          />
        </label>
      </div>
      <label className="admin-field">
        ...
        <Textarea
          ...
          placeholder="شرح تفصيلي لما يقدمه القسم"
        />
      </label>
    </ManagedForm>
  )
        ...
        <CardContent>
          ...
          <ManagedForm action={createSectionContent} submitLabel="حفظ المحتوى">
            <div className="admin-form-grid">
              ...
              <Input aria-label="العنوان" name="title" required minLength={2} placeholder="العنوان" />
              <Input aria-label="content-slug"
                name="slug"
                required
                dir="ltr"
                pattern="[a-z0-9]+(?:-[a-z0-9]+)*"
              ...
              />
              <Input aria-label="المستوى المستهدف" name="targetLevel" placeholder="المستوى المستهدف" />
              <Input aria-label="النقاط" name="points" type="number" min="0" defaultValue="0" placeholder="النقاط" />
              <Input aria-label="نسبة النجاح" name="passingScore" type="number" min="0" max="100" placeholder="نسبة النجاح" />
              <Input aria-label="الترتيب" name="sortOrder" type="number" min="0" defaultValue="0" placeholder="الترتيب" />
            ...
            <Textarea aria-label="ملخص المحتوى" name="summary" placeholder="ملخص المحتوى" />
            <Textarea aria-label="نص المحتوى" name="contentText" placeholder="نص المحتوى" />
            <Textarea aria-label="تعليمات المتعلم" name="instructions" placeholder="تعليمات المتعلم" />
              ...
              {data.contents.map((item) => (
                <tr key={item.id}>
                  ...
                  <td className="admin-inline-actions">
                    ...
                    >
                      <Input aria-label="سبب النسخ" name="reason" required minLength={5} placeholder="سبب النسخ" />
                    </ManagedForm>
                    <ManagedForm
                      compact
...
function ChallengesPanel({ data }: { data: Data }) {
  return (
    <div className="admin-stack">
      <Card>
        <CardHeader>
          <CardTitle>إنشاء تحدٍ</CardTitle>
        </CardHeader>
        <CardContent>
          <ManagedForm action={createManagedChallenge} submitLabel="حفظ التحدي">
            <div className="admin-form-grid">
              ...
              <label className="admin-field">
                <span>صورة التحدي</span>
                <select name="imageMediaId" defaultValue="">
              ...
              <label className="admin-field">
                <span>مقياس التقدم</span>
                <select name="metric" defaultValue="words">
                  <option value="words">عدد الكلمات المضافة</option>
                  <option value="sentences">عدد الجمل المضافة</option>
                  <option value="reviews">عدد المراجعات المكتملة</option>
                  <option value="streak">أيام الاستمرار</option>
                </select>
              </label>
              <label className="admin-field">
              ...
              <label className="admin-field">
                <span>مكافأة XP</span>
                <Input name="xpReward" type="number" min="0" defaultValue="0" />
              </label>
              <label className="admin-field">
                <span>مكافأة العملات</span>
                <Input name="coinReward" type="number" min="0" defaultValue="0" />
              </label>
              <label className="admin-field">
                <span>نوع التحدي</span>
                <select name="competitionType" defaultValue="progress">
              ...
              <label className="admin-field">
                ...
                <select name="status" defaultValue="draft">
                  <option value="draft">مسودة</option>
                  <option value="scheduled">مجدول</option>
                  <option value="open">مفتوح للانضمام</option>
                  <option value="active">نشط</option>
              ...
              <label className="admin-field">
                <span>الحد الأدنى للمشاركين</span>
                <Input name="minimumParticipants" type="number" min="1" defaultValue="1" />
              </label>
              <label className="admin-field">
                <span>الحد الأقصى للمشاركين (اختياري)</span>
                <Input name="maximumParticipants" type="number" min="1" />
              </label>
              <label className="admin-field">
                <span>عدد الفائزين</span>
                <Input name="winnerCount" type="number" min="1" defaultValue="1" />
              </label>
            </div>
            <label className="admin-field">
            ...
            <div className="admin-form-grid">
              <label className="admin-field">
                <span>الحد الأدنى من XP للمشاركة</span>
                <Input name="minimumXp" type="number" min="0" defaultValue="0" />
              </label>
              <label className="admin-field">
                <span>الجمهور المستهدف (اختياري)</span>
                <Input name="targetAudience" maxLength={120} />
                <small>مثال: طلاب المستوى المبتدئ.</small>
              </label>
              <label className="admin-field">
                <span>معامل احتساب النقاط</span>
                <Input name="scoreMultiplier" type="number" min="0.1" max="100" step="0.1" defaultValue="1" />
              </label>
              <label className="admin-field">
                <span>قاعدة كسر التعادل</span>
                <select name="tieBreaker" defaultValue="earliest_completion">
              ...
              <label className="admin-check-field">
                <input type="checkbox" name="requireVerifiedEmail" defaultChecked />
                <span>اشتراط بريد إلكتروني موثّق</span>
              </label>
            </div>
          </ManagedForm>
        </CardContent>
            ...
            <Card key={item.id}>
              ...
              <CardContent className="admin-stack-sm">
                ...
                >
                  ...
                  <Input aria-label="سبب الإنهاء أو الإلغاء" name="reason" placeholder="سبب الإنهاء أو الإلغاء" />
                </ManagedForm>
                <ManagedForm
                ...
                >
                  <Input aria-label="سبب اعتماد النتائج" name="reason" required minLength={5} placeholder="سبب اعتماد النتائج" />
                </ManagedForm>
                {leaders.length > 0 && (
    ...
    <div className="admin-stack">
      ...
      <div className="admin-card-grid">
        {data.competitions.map((competition) => {
          ...
          const mathSectionIds = data.competitionMathSections
            .filter((item) => item.competitionId === competition.id)
          ...
          return (
            <Card key={competition.id}>
              ...
              <CardContent className="admin-stack-sm">
                <div className="admin-row">
                  <Status value={competition.lifecycle} />
                  ...
                  <span>{competition.coinReward} عملة</span>
                ...
                >
                  <Input aria-label="سبب الحذف" name="reason" required minLength={5} placeholder="سبب الحذف" />
                </ManagedForm>
                {participants.length > 0 && (
                  <details>
                        ...
                        <div className="admin-list-row" key={participant.id}>
                          ...
                          {['joined', 'active', 'disqualified'].includes(participant.status) && (
                            ...
                            >
                              ...
                              />
                              <Input aria-label="سبب تغيير الحالة" name="reason" required minLength={5} placeholder="سبب تغيير الحالة" />
                            </ManagedForm>
                          )}
                        </div>
                      ))}
...
function CompetitionForm({
  ...
  return (
    ...
    >
      <div className="admin-form-grid">
        <label className="admin-field">
          <span>عنوان المنافسة</span>
          <Input name="title" required minLength={2} defaultValue={competition?.title} />
        </label>
        <label className="admin-field">
          <span>النطاق</span>
          <select name="scope" defaultValue={competition?.scope ?? 'all'}>
        ...
        <label className="admin-field">
          ...
          <select name="lifecycle" defaultValue={competition?.lifecycle ?? 'draft'}>
            <option value="draft">مسودة</option>
        ...
        <label className="admin-field">
          ...
          <select name="audience" defaultValue={competition?.audience ?? 'all'}>
            <option value="all">كل المستخدمين</option>
            <option value="verified">الحسابات الموثقة</option>
            <option value="new_users">المستخدمون الجدد خلال 30 يومًا</option>
            <option value="existing_users">المستخدمون الأقدم من 30 يومًا</option>
          </select>
        </label>
        <label className="admin-field">
        ...
        <label className="admin-field">
          <span>مكافأة XP القصوى</span>
          <Input name="xpReward" type="number" min="0" defaultValue={competition?.xpReward ?? 100} />
        </label>
        <label className="admin-field">
          <span>مكافأة العملات القصوى</span>
          <Input name="coinReward" type="number" min="0" defaultValue={competition?.coinReward ?? 0} />
        </label>
        <label className="admin-field">
          <span>الأولوية</span>
          <Input name="priority" type="number" min="1" defaultValue={competition?.priority ?? 100} />
        </label>
        <label className="admin-field">
          <span>موعد البداية</span>
          <Input name="startsAt" type="datetime-local" defaultValue={localInputDate(competition?.startsAt ?? null)} />
        </label>
        <label className="admin-field">
          <span>موعد النهاية</span>
          <Input name="endsAt" type="datetime-local" defaultValue={localInputDate(competition?.endsAt ?? null)} />
        </label>
        <label className="admin-check-field">
          <input
            type="checkbox"
            name="requiresVerifiedEmail"
            defaultChecked={competition?.requiresVerifiedEmail ?? false}
          />
          <span>اشتراط بريد موثّق</span>
        </label>
      </div>
      <fieldset className="admin-choice-group">
        <legend>أقسام الرياضيات</legend>
        <p>تُستخدم في منافسات الرياضيات والمنافسات المختلطة. يمكن اختيار أكثر من قسم.</p>
        <div className="admin-checkbox-grid">
          {data.mathSections.map((section) => (
            <label key={section.id} className="admin-check-field">
              <input
                type="checkbox"
                name="mathSectionIds"
                value={section.id}
                defaultChecked={mathSectionIds.includes(section.id) || competition?.sectionId === section.id}
                disabled={!section.isActive}
              />
              <span>{section.name}{section.isActive ? '' : ' (معطل)'}</span>
            </label>
          ))}
          {!data.mathSections.length && <span>لا توجد أقسام رياضيات متاحة.</span>}
        </div>
      </fieldset>
      <fieldset className="admin-choice-group">
        <legend>أقسام أسئلة اللغات</legend>
        <p>اختر أقسام العربية أو الإنجليزية المناسبة. النطاق المختلط يقبل اللغتين.</p>
        <div className="admin-checkbox-grid">
          {data.platformCategories.map((category) => (
            <label key={category.id} className="admin-check-field">
              <input
                type="checkbox"
                name="categoryIds"
                value={category.id}
              ...
              />
              <span>{category.name} — {category.language === 'ar' ? 'عربي' : 'إنجليزي'}</span>
            </label>
          ))}
          {!data.platformCategories.length && <span>لا توجد أقسام أسئلة لغوية متاحة.</span>}
        </div>
      </fieldset>
      <label className="admin-field">
        <span>الوصف</span>
      ...
      <label className="admin-field">
        <span>القواعد والتعليمات</span>
        <Textarea
          name="rules"
          defaultValue={competition?.rules ?? ''}
          placeholder="اشرح مدة المنافسة وطريقة احتساب النتائج وأي شروط إضافية"
        />
      </label>
      {requireReason && (
        <label className="admin-field">
        ...
        <CardContent>
          <ManagedForm action={createLevel} submitLabel="إضافة مستوى">
            <div className="admin-form-grid">
              ...
              <label className="admin-field">
                <span>ترتيب المستوى</span>
                <Input name="rank" type="number" min="1" required />
              </label>
              <label className="admin-field">
                <span>الحد الأدنى من XP</span>
                <Input name="minimumPoints" type="number" min="0" defaultValue="0" />
              </label>
              <label className="admin-field">
                <span>لون المستوى</span>
                <div className="admin-color-field">
              ...
              <label className="admin-check-field">
                ...
                <span>المستوى نشط</span>
              </label>
            </div>
            <label className="admin-field">
              <span>وصف المستوى ومعايير الوصول إليه</span>
          ...
          {data.levels.map((level) => (
            <div className="admin-list-row" key={level.id}>
              <div>
                ...
                <span>{level.minimumPoints} XP · {level.userCount} مستخدم · {level.description || 'بلا وصف'}</span>
              </div>
              ...
              <details>
                ...
                >
                  ...
                  <label className="admin-check-field">
                    <input type="checkbox" name="isActive" defaultChecked={level.isActive} />
                    <span>نشط</span>
                  </label>
                  <label className="admin-field">
                    <span>الوصف</span>
      ...
      <div className="admin-kpis">
        {data.levels.map((level) => (
          <Kpi
            ...
            value={`${level.userCount} مستخدم`}
            hint={`${level.minimumPoints} XP للترقية`}
          />
        ))}
      </div>
      <Card>
        ...
        <CardContent>
          <ManagedForm action={createPromotionRule} submitLabel="إنشاء القاعدة">
            <div className="admin-form-grid">
              <Input aria-label="اسم القاعدة" name="name" required minLength={3} placeholder="اسم القاعدة" />
              ...
              <Input aria-label="قيمة الشرط الأول" name="threshold1" type="number" min="0" required placeholder="قيمة الشرط الأول" />
              ...
              <Input aria-label="قيمة الشرط الثاني"
                name="threshold2"
                type="number"
                min="0"
                defaultValue="0"
              ...
              <Input aria-label="قيمة الشرط الثالث"
                name="threshold3"
                type="number"
                min="0"
                defaultValue="0"
              ...
              <Input aria-label="إضافة XP" name="xpAmount" type="number" min="0" defaultValue="0" placeholder="إضافة XP" />
              <Input aria-label="إضافة عملات" name="coinAmount" type="number" min="0" defaultValue="0" placeholder="إضافة عملات" />
              ...
              <Input aria-label="الأولوية" name="priority" type="number" min="1" defaultValue="100" placeholder="الأولوية" />
              <label>
            ...
            <Textarea aria-label="وصف القاعدة" name="description" placeholder="وصف القاعدة" />
          </ManagedForm>
        </CardContent>
      </Card>
      <Card>
            ...
            <div className="admin-list-row" key={rule.id}>
              ...
              {!rule.archivedAt && (
                <details>
                  ...
                  >
                    <Input aria-label="الاسم" name="name" required minLength={3} defaultValue={rule.name} />
                    <Textarea aria-label="الوصف" name="description" defaultValue={rule.description ?? ''} />
                    <Input aria-label="تاريخ البداية"
                      name="startsAt"
                      type="datetime-local"
                      defaultValue={localInputDate(rule.startsAt)}
                    />
                    <Input aria-label="تاريخ النهاية" name="endsAt" type="datetime-local" defaultValue={localInputDate(rule.endsAt)} />
                    <select aria-label="نطاق التطبيق" name="applicationMode" defaultValue={rule.applicationMode}>
                      <option value="new_only">الإنجازات الجديدة فقط</option>
                      <option value="all_matching">كل المستخدمين المطابقين</option>
                    </select>
                    <Input aria-label="الأولوية" name="priority" type="number" min="1" defaultValue={rule.priority} />
                    <label>
              ...
              {!rule.archivedAt && rule.retrospective && (
                ...
                >
                  <Input aria-label="سبب التنفيذ الرجعي" name="reason" required minLength={5} placeholder="سبب التنفيذ الرجعي" />
                </ManagedForm>
              )}
              <ManagedForm
                compact
              ...
              >
                <Input aria-label="سبب العملية" name="reason" required minLength={5} placeholder="سبب العملية" />
              </ManagedForm>
            </div>
          ))}
          ...
          {data.ruleExecutions.map((execution) => (
            <div className="admin-list-row" key={execution.id}>
              ...
              {execution.status === 'failed' && (
                ...
                >
                  <Input aria-label="سبب إعادة المحاولة" name="reason" required minLength={5} placeholder="سبب إعادة المحاولة" />
                </ManagedForm>
              )}
            </div>
          ))}
      ...
      <Card>
        ...
        <CardContent>
          <ManagedForm action={createBadge} submitLabel="حفظ الشارة">
            <div className="admin-form-grid">
              <Input aria-label="اسم الشارة" name="name" required minLength={2} placeholder="اسم الشارة" />
              <Input aria-label="badge-slug"
                name="slug"
                required
                dir="ltr"
                pattern="[a-z0-9]+(?:-[a-z0-9]+)*"
              ...
              <Input aria-label="قيمة الشرط" name="threshold" type="number" min="0" defaultValue="0" placeholder="قيمة الشرط" />
              <label>
            ...
            <Textarea aria-label="الوصف" name="description" required minLength={3} placeholder="الوصف" />
          </ManagedForm>
        </CardContent>
      </Card>
      <div className="admin-card-grid">
      ...
      <Card>
        ...
        <CardContent>
          <ManagedForm action={createAchievement} submitLabel="حفظ الإنجاز">
            <div className="admin-form-grid">
              <Input aria-label="اسم الإنجاز" name="name" required minLength={2} placeholder="اسم الإنجاز" />
              <Input aria-label="رمز أو اسم أيقونة" name="icon" maxLength={120} placeholder="رمز أو اسم أيقونة" />
              <Input aria-label="مكافأة XP" name="xpReward" type="number" min="0" defaultValue="0" placeholder="مكافأة XP" />
              <Input aria-label="العملات" name="coinReward" type="number" min="0" defaultValue="0" placeholder="العملات" />
              ...
              <Input aria-label="قيمة الشرط" name="requirementValue" type="number" min="1" required placeholder="قيمة الشرط" />
              <Input aria-label="الترتيب" name="sortOrder" type="number" min="0" defaultValue="0" placeholder="الترتيب" />
              <label>
                <input type="checkbox" name="isActive" defaultChecked /> نشط
              </label>
            </div>
            <Textarea aria-label="وصف الإنجاز" name="description" required minLength={3} placeholder="وصف الإنجاز" />
          </ManagedForm>
        </CardContent>
      </Card>
      <Card>
        ...
        <CardContent className="admin-list">
          {data.siteContent.map((item) => (
            <div className="admin-list-row" key={item.id}>
              ...
              >
                <Input aria-label="سبب تغيير الظهور" name="reason" required minLength={5} placeholder="سبب تغيير الظهور" />
              </ManagedForm>
              {item.status !== 'published' && (
                <ManagedForm
                  compact
                ...
                >
                  <Input aria-label="سبب النشر" name="reason" required minLength={5} placeholder="سبب النشر" />
                </ManagedForm>
              )}
              {item.status === 'published' && (
                <ManagedForm
                ...
                >
                  <Input aria-label="سبب إلغاء النشر" name="reason" required minLength={5} placeholder="سبب إلغاء النشر" />
                </ManagedForm>
              )}
  ...
  return (
    ...
    >
      <div className="admin-form-grid">
        ...
        <label className="admin-field">
          <span>المفتاح التقني الفريد</span>
          <Input
            name="key"
            required
            dir="ltr"
          ...
          />
          <small>يُستخدم لربط العنصر بموضعه في الواجهة ولا يمكن تغييره بعد الإنشاء.</small>
        </label>
        <label className="admin-field">
          <span>مجموعة العرض</span>
          <Input
            name="group"
            required
            dir="ltr"
        ...
        <label className="admin-field">
          <span>العنوان الإداري</span>
          <Input name="title" defaultValue={item?.title ?? ''} placeholder="اسم يظهر للمدير" />
        </label>
        <label className="admin-field">
          <span>العنوان الظاهر للمستخدم</span>
          <Input
            name="heading"
        ...
        <label className="admin-field">
          ...
          <Input
            ...
            placeholder="/learn"
          />
          <small>رابط داخلي يبدأ بشرطة مائلة أو رابط HTTPS.</small>
        </label>
        <label className="admin-field">
          <span>نص الزر الإضافي</span>
          <Input
        ...
        <label className="admin-field">
          <span>نمط العرض</span>
          <select name="styleVariant" defaultValue={stringFromRecord(content, 'styleVariant') || 'default'}>
        ...
        <label className="admin-field">
          <span>صورة المحتوى</span>
          <select name="imageMediaId" defaultValue={stringFromRecord(content, 'imageMediaId')}>
            <option value="">بلا صورة</option>
            {media
        ...
        <label className="admin-field">
          ...
          <Input name="sortOrder" type="number" min="0" defaultValue={item?.sortOrder ?? 0} />
        </label>
        <label className="admin-field">
          <span>بداية العرض (اختياري)</span>
          <Input name="startsAt" type="datetime-local" defaultValue={localInputDate(item?.startsAt ?? null)} />
        </label>
        <label className="admin-field">
          <span>نهاية العرض (اختياري)</span>
          <Input name="endsAt" type="datetime-local" defaultValue={localInputDate(item?.endsAt ?? null)} />
        </label>
        <label className="admin-check-field">
          <input type="checkbox" name="isVisible" defaultChecked={item?.isVisible ?? true} />
          <span>إظهار العنصر ضمن شروط النشر والموعد</span>
        </label>
      </div>
      <label className="admin-field">
        <span>النص الرئيسي الظاهر للزائر</span>
        <Textarea
          name="text"
          maxLength={5000}
...
function MediaPanel({ data }: { data: Data }) {
  ...
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setResult({ ok: false, message: publicActionError(error, 'تعذر الرفع.') })
    } finally {
      setPending(false)
    }
  }
  async function remove(id: string) {
    ...
    try {
      ...
      setResult({ ok: false, message: publicActionError(error, 'تعذر الحذف.') })
    } finally {
      setPending(false)
    }
  }
  async function replace(id: string, file: File | undefined) {
    ...
    try {
      ...
      setResult({ ok: false, message: publicActionError(error, 'تعذر استبدال الملف.') })
    } finally {
      setPending(false)
    }
  }
  ...
  async function updateMetadata(event: FormEvent<HTMLFormElement>, id: string) {
    ...
    try {
      ...
      setResult({ ok: false, message: publicActionError(error, 'تعذر حفظ بيانات الملف.') })
    } finally {
      setPending(false)
    }
  }
      ...
      <Card>
        ...
        <CardContent>
          <form className="admin-filter-form" method="get" action="/admin/media">
            <Input aria-label="اسم الملف أو وصفه" name="search" defaultValue={params.get('search') ?? ''} placeholder="اسم الملف أو وصفه" />
      ...
      <Card>
        ...
        <CardContent>
          <form onSubmit={upload} className="admin-form">
            ...
            />
            <Input aria-label="النص البديل أو وصف الملف" name="altText" maxLength={500} placeholder="النص البديل أو وصف الملف" />
            <Input aria-label="عنوان الملف (اختياري)" name="title" maxLength={200} placeholder="عنوان الملف (اختياري)" />
            <Textarea aria-label="وصف الملف (اختياري)" name="description" maxLength={2000} placeholder="وصف الملف (اختياري)" />
          ...
          <Card key={item.id}>
            ...
            <CardContent className="admin-stack">
              <form className="admin-form" onSubmit={(event) => void updateMetadata(event, item.id)}>
                <Input aria-label="العنوان" name="title" maxLength={200} defaultValue={item.title ?? ''} placeholder="العنوان" />
                <Input aria-label="النص البديل"
                  name="altText"
                  maxLength={500}
                  defaultValue={item.altText ?? ''}
                  placeholder="النص البديل"
                />
                <Textarea aria-label="الوصف"
                  name="description"
                  maxLength={2000}
  ...
  return (
    <div className="admin-stack">
      ...
      <Card>
        ...
        <CardContent className="admin-metric-list">
          <Metric label="الحساب الحالي" value={data.actor.email} />
          <Metric label="الدور المطلوب" value="admin محفوظ في قاعدة البيانات" />
          <Metric label="توثيق البريد" value="إلزامي" />
          <Metric label="مصدر الصلاحية" value="الدور والحالة وتوثيق البريد على الخادم" />
        </CardContent>
      </Card>
      <Alert>
        <AlertDescription>
          إضافة مدير جديد تتم بتعديل دور الحساب من مسار إداري محمي أو بأداة الترقية الآمنة. لا توجد صلاحية قابلة للحقن من الواجهة، ولا يعتمد الوصول على بريد مكتوب داخل الكود.
        </AlertDescription>
      </Alert>
    </div>
  )
...
function BackupPanel({ data }: { data: Data }) {
  ...
  const deliveryLabel: Record<string, string> = {
    download: 'تنزيل آمن',
    'email-attachment': 'مرفق بريد',
    'email-link': 'رابط بريد مؤقت',
  }
  const numberFormatter = new Intl.NumberFormat('ar-EG')
  const byteFormatter = new Intl.NumberFormat('ar-EG', { maximumFractionDigits: 1 })
  const formatBytes = (value: number) => {
    if (value < 1024) return `${numberFormatter.format(value)} بايت`
    if (value < 1024 * 1024) return `${byteFormatter.format(value / 1024)} كيلوبايت`
    return `${byteFormatter.format(value / 1024 / 1024)} ميجابايت`
  }
  return (
    <div className="admin-stack">
      <Alert>
        <AlertDescription>
          نسخة البيانات والملفات التشغيلية تشمل الجداول المسموح بها وملفات Vercel Blob. تُحفظ تجزئات كلمات المرور اللازمة للاستعادة في ملف مشفّر مستقل، وتُستبعد الجلسات ورموز التحقق والاسترداد ومفاتيح البيئة.
        </AlertDescription>
      </Alert>
      <BackupActions />
      <Card>
        <CardHeader>
          ...
          <CardDescription>آخر 50 عملية طلبها حساب المدير الحالي، بحالتها وأعدادها الفعلية.</CardDescription>
        </CardHeader>
        <CardContent className="admin-stack-sm">
          {!data.backupJobs.length && <p className="muted">لا توجد عمليات نسخ احتياطي مسجلة بعد.</p>}
          {data.backupJobs.map((job) => (
            <article className="admin-list-card" key={job.id}>
              <div className="admin-row admin-row-between">
                <div>
                  <strong>{job.filename ?? 'نسخة قيد التجهيز'}</strong>
                  <p className="muted">
              ...
              <div className="admin-metric-list compact">
                <Metric label="طريقة التسليم" value={deliveryLabel[job.delivery] ?? job.delivery} />
                <Metric label="السجلات" value={numberFormatter.format(job.recordCount)} />
                <Metric label="ملفات التخزين" value={numberFormatter.format(job.blobCount)} />
                <Metric label="الحجم" value={formatBytes(job.sizeBytes)} />
                {job.recipient && <Metric label="المستلم" value={job.recipient} />}
              </div>
              {job.errorMessage && <p className="admin-error-text">{job.errorMessage}</p>}
            </article>
          ))}
        </CardContent>
      </Card>
  ...
  function download() {
    startTransition(async () => {
      try {
        ...
        setResult({ ok: false, message: publicActionError(error, 'تعذر إنشاء النسخة.') })
      }
    })
  }
  function email() {
    startTransition(async () => {
      try {
        ...
        setResult({ ok: false, message: publicActionError(error, 'تعذر إرسال النسخة.') })
      }
    })
  }
  return (
    <Card>
      <CardHeader>
        <CardTitle>نسخة البيانات والملفات التشغيلية</CardTitle>
        <CardDescription>
          أنشئ نسخة ZIP قابلة للحفظ أو أرسلها إلى البريد المحدد في BACKUP_RECIPIENT_EMAIL. عند غياب المتغير يُستخدم بريد حساب المدير الحالي. إذا كان الحجم كبيرًا، تصل الرسالة برابط تنزيل آمن صالح 7 أيام.
        </CardDescription>
      </CardHeader>
      <CardContent className="admin-stack-sm">
        {result && <Feedback result={result} />}
        ...
        onClick={() =>
          startTransition(async () => {
            try {
              ...
              setResult({
                ok: false,
                message: publicActionError(error, 'تعذر تنفيذ العملية.'),
              })
            }
          })
        }
