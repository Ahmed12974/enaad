...
test('verified database administrator opens admin routes and APIs while unverified or banned accounts are rejected', async ({ page }) => {
  test.skip(
    process.env.E2E_ALLOW_ADMIN_FIXTURE !== '1',
    'Requires an isolated test database for the administrator fixture.',
  )
  const databaseUrl = process.env.DATABASE_URL
  if (!databaseUrl) throw new Error('DATABASE_URL is required for the administrator E2E test.')
  const pool = new Pool({ connectionString: databaseUrl, max: 1 })
  const suffix = randomUUID()
  const email = `admin-auth-${suffix}@example.test`
  const password = `E2E-${suffix}-aA9!`
  try {
    await pool.query(`delete from "user" where email = $1`, [email])
    await page.goto('/sign-up')
    await page.getByLabel('الاسم').fill('مدير اختبار')
    await page.getByLabel('البريد الإلكتروني').fill(email)
    await page.getByLabel('كلمة المرور').fill(password)
    await page.getByRole('button', { name: 'إنشاء الحساب' }).click()
    await page.waitForURL((url) => url.pathname === '/')
    ...
    expect((await page.request.get('/api/admin/reports/users')).status()).toBe(200)
    ...
    await pool.query(`update "user" set banned = false, role = 'user' where id = $1`, [adminId])
    await page.goto('/admin')
    await expect(page.getByRole('heading', { name: /403 — الوصول مرفوض/ })).toBeVisible()

    await pool.query(`update "user" set role = 'admin' where id = $1`, [adminId])
    await page.goto('/admin')
    await expect(page.getByRole('heading', { name: 'لوحة التحكم' })).toBeVisible()
  } finally {
    await pool.query(`delete from "user" where email = $1`, [email])
