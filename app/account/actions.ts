...
const genericResetMessage = {
  ok: true,
  message:
    'إذا كانت البيانات مطابقة لحساب وكانت خدمة البريد متاحة، فستصلك رسالة الاسترداد خلال دقائق.',
}
  ...
  try {
    ...
    await db.transaction(async (tx) => {
      await tx
        .update(recoveryTokens)
        .set({ usedAt: new Date() })
        .where(and(eq(recoveryTokens.tokenHash, tokenHash), isNull(recoveryTokens.usedAt)))
      await tx
        .update(user)
        ...
        .where(
          and(
            ...
            eq(user.recoveryEmail, email),
            eq(user.recoveryEmailVerified, false),
          ),
        )
    })
    throw error
...
export async function consumePasswordReset(token: string, password: string) {
  ...
  return db.transaction(async (tx) => {
    ...
    const [claimed] = await tx
      .update(recoveryTokens)
      .set({ usedAt: now })
      .where(and(eq(recoveryTokens.id, record.id), isNull(recoveryTokens.usedAt)))
      .returning({ id: recoveryTokens.id })
    if (!claimed) return { ok: false, message: 'تم استخدام رابط الاسترداد بالفعل.' }
    await tx
      .update(account)
