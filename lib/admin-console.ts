...
export async function getAdminConsoleData(filters: AdminFilters = {}, section: AdminSection = 'overview') {
  ...
  const needsBadges =
    ...
    section === 'promotions' ||
    section === 'statistics'
  const needsBackups = section === 'backup' || section === 'overview'
  ...
  const [
    ...
    executeWhen(needsSections, () =>
      ...
      select s.id, s.name, s."nameEn", s.slug, s."sectionType", s."previousSectionId",
        s."shortDescription", s."fullDescription", s."iconMediaId", s."imageMediaId", s.color,
        s."ageRange", s."targetLevel", s."unlockRules", s.access, s."isPaid", s."priceMinor",
        s.currency, s.status, s."isActive", s."sortOrder",
    ...
    executeWhen(needsLevels, () =>
      db.execute<{
        ...
        select l.*, count(p."userId")::int as "userCount"
        from levels l
        left join "userAdminProfiles" p on p."levelId" = l.id
        group by l.id
        order by l.rank
        limit 100
      `),
  ...
  const [competitionRows, competitionCategoryRows, competitionMathSectionRows, competitionParticipantRows, platformCategoryRows, mathSectionRows] =
    await Promise.all([
      executeWhen(needsCompetitions, () =>
        db.execute<{
          ...
          lifecycle: 'draft' | 'scheduled' | 'active' | 'ended' | 'cancelled'
          rules: string | null
          audience: 'all' | 'verified' | 'new_users' | 'existing_users'
          priority: number
          ...
          select c.id, c.title, c.description, c.scope, c."sectionId", c.lifecycle, c.rules, c.audience,
            c.priority, c."requiresVerifiedEmail", c."questionCount", c."xpReward", c."coinReward",
      ...
      selectWhen(section === 'competitions', () =>
        db.select().from(competitionMathSections).orderBy(asc(competitionMathSections.competitionId)),
      ),
      selectWhen(section === 'competitions', () =>
        db
          .select({
  ...
  const [archivedSections, archivedContents, backupJobRows] = await Promise.all([
    ...
    selectWhen(needsBackups, () =>
      db
        .select({
          id: backupJobs.id,
          status: backupJobs.status,
          delivery: backupJobs.delivery,
          recipient: backupJobs.recipient,
          filename: backupJobs.filename,
          recordCount: backupJobs.recordCount,
          blobCount: backupJobs.blobCount,
        ...
        .from(backupJobs)
        .where(eq(backupJobs.requestedBy, actor.id))
