...
export async function synchronizeUserLevel(tx: DbTransaction, userId: string) {
  ...
  const [level] = await tx
    ...
    .from(levels)
    .where(and(eq(levels.isActive, true), lte(levels.minimumPoints, progress.xp)))
    .orderBy(desc(levels.minimumPoints), desc(levels.rank))
    .limit(1)
  await tx
    .insert(userAdminProfiles)
    .values({ userId, levelId: level?.id ?? null })
    .onConflictDoUpdate({
      target: userAdminProfiles.userId,
      set: { levelId: level?.id ?? null, updatedAt: new Date() },
    })
  if (level) {
    await tx
      .update(userProgress)
      .set({ title: level.name, updatedAt: new Date() })
      .where(eq(userProgress.userId, userId))
  }
  return level ?? null
}

export async function synchronizeAllUserLevels(tx: DbTransaction) {
  const rows = await tx.select({ userId: userProgress.userId }).from(userProgress)
  for (const row of rows) await synchronizeUserLevel(tx, row.userId)
  return rows.length
}
...
export async function touchUserActivity(tx: DbTransaction, userId: string) {
  ...
  await synchronizeUserLevel(tx, userId)
...
export async function awardProgress(
  ...
  await synchronizeUserLevel(tx, input.userId)
