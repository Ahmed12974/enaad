...
test('sole administrator completes content, CMS, rules, reports, media validation and settings flows', async ({
  page,
}, testInfo) => {
  test.skip(
    process.env.E2E_ALLOW_ADMIN_FIXTURE !== '1',
    'Requires an isolated PostgreSQL test database and the sole-admin fixture flag.',
  )
  ...
  try {
    ...
    const sectionForm = page
      .locator('form')
      .filter({ has: page.getByPlaceholder('اسم القسم') })
      .first()
    await sectionForm.getByPlaceholder('اسم القسم').fill(`قسم ${suffix}`)
    await sectionForm.getByPlaceholder('section-slug').fill(sectionSlug)
    ...
    await pool.query(`delete from "adminAllowlist" where email = $1`, [email]).catch(() => undefined)
