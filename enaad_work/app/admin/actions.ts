...
export async function createEducationalSection(form: FormData): Promise<Result> {
  ...
  try {
    ...
    if (isUniqueViolation(error)) return { ok: false, message: 'المعرّف مستخدم في قسم آخر.' }
    throw error
  }
}
...
export async function updateEducationalSection(sectionId: string, form: FormData): Promise<Result> {
  ...
  await db.transaction(async (tx) => {
    ...
    const sectionGraph = await tx
      .select({ id: educationalSections.id, unlockRules: educationalSections.unlockRules })
      .from(educationalSections)
      .where(isNull(educationalSections.deletedAt))
    const prerequisiteById = new Map(
      sectionGraph.map((candidate) => {
        const rules = candidate.unlockRules
        const prerequisite =
          rules && typeof rules === 'object' && !Array.isArray(rules)
            ? (rules as Record<string, unknown>).prerequisiteSectionId
            : null
        return [candidate.id, typeof prerequisite === 'string' ? prerequisite : null] as const
      }),
    )
    const requestedPrerequisite = parsed.unlockRules.prerequisiteSectionId
    if (requestedPrerequisite && !prerequisiteById.has(requestedPrerequisite)) {
      throw new Error('القسم السابق المطلوب غير موجود.')
    }
    prerequisiteById.set(id, requestedPrerequisite)
...
export async function createManagedChallenge(form: FormData): Promise<Result> {
  ...
  const parsed = z
    .object({
      ...
      status: lifecycle,
      competitionType: z.string().trim().min(2).max(80),
...
export async function grantUserAchievement(form: FormData): Promise<Result> {
  ...
  const granted = await db.transaction(async (tx) => {
    ...
    if (ledger)
      await tx
        .insert(userProgress)
        .values({ userId, xp: achievement.xpReward, coins: achievement.coinReward })
        .onConflictDoUpdate({
          target: userProgress.userId,
          set: {
    ...
    await writeAdminAudit(
      tx,
      {
        ...
        entityType: 'achievement',
        entityId: String(achievementId),
        after: { userId, rewardGranted: Boolean(ledger) },
        reason,
      },
      context,
    )
