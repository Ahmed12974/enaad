...
import { appRoles, hasPermission, type Permission } from '@/lib/admin'
...
export async function getData() {
  ...
  const [
    ...
    db
      ...
      .limit(1_000),
    db.select().from(tests).where(eq(tests.userId, current.id)).orderBy(desc(tests.completedAt)).limit(100),
    ...
    db
      ...
      .limit(100),
    db.select().from(challengeParticipants).where(eq(challengeParticipants.userId, current.id)),
    db
      .select()
      .from(achievements)
      ...
      .limit(200),
    db.select().from(userAchievements).where(eq(userAchievements.userId, current.id)),
    db
      .select()
      .from(banners)
      .where(
    ...
    db
      ...
      .where(eq(encouragementMessages.isActive, true))
      .orderBy(asc(encouragementMessages.sortOrder))
      .limit(100),
    db
  ...
  const participantCountRows = challengeRows.length
    ? await db
...
export async function updateWord(id: number, form: FormData) {
  ...
  try {
    const [updated] = await db
      .update(words)
      .set({ ...parsed.data, categoryId, word, normalizedWord: normalizeText(word), updatedAt: new Date() })
...
export async function importMathWorkbook(sectionSlug: string, form: FormData) {
  ...
  for (const file of files) {
    ...
    const header = workbookRows[0].map((cell) => {
...
async function competitionWindow(competition: typeof competitions.$inferSelect) {
  const now = new Date()
  if (!competition.isActive) throw new Error('المنافسة غير متاحة')
  if (competition.startsAt && competition.startsAt > now) throw new Error('لم تبدأ المنافسة بعد')
  if (competition.endsAt && competition.endsAt < now) throw new Error('انتهت المنافسة')
}

export async function createCompetition(form: FormData) {
  const actor = await requirePermission('competitions:write')
  const parsed = z
    .object({
  ...
  try {
    const [created] = await db.transaction(async (tx) => {
      const records = await tx
        .insert(competitions)
        .values({ ...parsed, normalizedTitle })
        .returning()
      if (records[0])
        await writeAudit(tx, actor.id, 'competition.create', 'competition', String(records[0].id), null, {
          title: records[0].title,
          scope: records[0].scope,
        })
      return records
    })
    refresh('/admin', '/competitions')
    return { ok: true, message: 'تم نشر المنافسة بنجاح.', competition: created }
  } catch (error) {
    if (isUniqueViolation(error)) return { ok: false, message: 'توجد منافسة بهذا العنوان بالفعل.' }
    throw error
  }
}
...
export async function joinCompetition(competitionId: number) {
  ...
  await competitionWindow(competition)
...
export async function submitCompetition(competitionId: number, answerValues: CompetitionAnswer[]) {
  ...
  return db.transaction(async (tx) => {
    ...
    if (
      ...
      (competition.startsAt && competition.startsAt > now) ||
      (competition.endsAt && competition.endsAt < now)
    ) {
      await tx
        .update(competitionParticipants)
        .set({ status: 'expired', updatedAt: now })
        .where(eq(competitionParticipants.id, state.id))
      throw new Error('انتهت مهلة تسليم هذه المنافسة.')
    }
...
export async function setUserRole(targetUserId: string, roleValue: string) {
  const actor = await requirePermission('users:write')
  const targetId = z.string().min(1).max(200).parse(targetUserId)
  const role = z.enum(appRoles).parse(roleValue)
  if (actor.id === targetId && role !== 'admin') throw new Error('لا يمكنك إزالة صلاحيتك الإدارية بنفسك.')
  await db.transaction(async (tx) => {
    const [before] = await tx
    ...
    await writeAudit(tx, actor.id, 'user.role.update', 'user', targetId, before, { role })
  })
  refresh('/admin')
}
