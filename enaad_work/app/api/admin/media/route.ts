...
function errorResponse(error: unknown) {
  ...
  const message = error instanceof Error ? error.message : 'تعذر تنفيذ طلب الوسائط.'
  const safeMessage = /token|secret|database|stack|blob/i.test(message) ? 'تعذر تنفيذ طلب الوسائط.' : message
...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      await del(uploaded.pathname, { token: process.env.BLOB_READ_WRITE_TOKEN }).catch(() => undefined)
      throw error
    }
    if (previousPathname)
      await del(previousPathname, { token: process.env.BLOB_READ_WRITE_TOKEN }).catch(() => undefined)
    return NextResponse.json({ ok: true, media: saved }, { status: replaceId ? 200 : 201 })
  } catch (error) {
    return errorResponse(error)
  }
}
