...
الحساب المالك الوحيد هو `enaadx@gmail.com`. بعد إنشاء الحساب أو تسجيل الدخول به، يقوم الخادم من داخل الجلسة الموثقة بمزامنة الدور `admin` وتفعيل سجل `SUPER_ADMIN` تلقائيًا، ثم يوجّه الحساب إلى `/admin`. لا تحتاج إلى تنفيذ SQL يدوي أو إضافة متغير `ADMIN_EMAIL`.
...
من `/admin` افتح قسم **النسخ الاحتياطي** ثم اضغط **إنشاء وإرسال إلى enaadx@gmail.com**.
...
الإرسال إلى `enaadx@gmail.com` لن ينجح باستخدام `onboarding@resend.dev` إلا إذا كان هذا البريد هو بريد حساب Resend نفسه؛ للاستخدام الحقيقي يجب توثيق نطاق تملكه في Resend.
