...
- اضبط `RESEND_API_KEY` و`EMAIL_FROM` لتفعيل رسالة تأكيد البريد والاسترداد وإرسال النسخة الاحتياطية. يجب أن يستخدم `EMAIL_FROM` نطاقًا موثّقًا في Resend، وليس `resend.dev`.
- اضبط `BLOB_READ_WRITE_TOKEN` لملفات البانرات والشهادات والنسخ الاحتياطية الخاصة.
...
منفصلة مع checksum وقفل استشاري، جميع الترحيلات `0003`–`0012`، ثم يتحقق من الجداول
...
البريد الوحيد المسموح به إداريًا هو `enaad4786@gmail.com`. لا يكفي الدور وحده: كل صفحة
...
- تبويب النسخ الاحتياطي ينشئ ملف ZIP يحتوي بيانات الجداول التشغيلية الآمنة وكل ملفات Vercel Blob، ويرسله إلى `enaad4786@gmail.com` كمرفق أو كرابط تنزيل خاص صالح 7 أيام عند زيادة الحجم. يستبعد كلمات المرور والجلسات والرموز السرية ومفاتيح البيئة، وتبقى ملفات الكود المصدرية محفوظة في GitHub. احتفظ أيضًا بـPoint-in-Time Recovery من مزود PostgreSQL واختبر الاستعادة دوريًا.
...
## تفعيل لوحة التحكم تلقائيًا

الحساب المالك الوحيد هو `enaad4786@gmail.com`. بعد إنشاء الحساب أو تسجيل الدخول به، يقوم الخادم من داخل الجلسة الموثقة بمزامنة الدور `admin` وتفعيل سجل `SUPER_ADMIN` تلقائيًا، ثم يوجّه الحساب إلى `/admin`. لا تحتاج إلى تنفيذ SQL يدوي أو إضافة متغير `ADMIN_EMAIL`.

أقسام لوحة التحكم المتاحة: الإحصائيات، المستخدمون، الأقسام، المحتوى، التحديات والمنافسات، المستويات والترقيات، الشارات والإنجازات، محتوى الواجهة، الوسائط، سجل العمليات، النسخ الاحتياطي، والإعدادات.

## النسخة الاحتياطية الكاملة من لوحة التحكم

من `/admin` افتح قسم **النسخ الاحتياطي** ثم اضغط **إنشاء وإرسال إلى enaad4786@gmail.com**.
النسخة تتضمن `database.json` و`manifest.json` وجميع ملفات Blob داخل مجلد `blobs/`. إذا كان الملف أقل من حد البريد يصل كمرفق ZIP؛ وإذا كان أكبر يُرفع إلى Private Blob ويصل رابط موقّع صالح 7 أيام.

متطلبات التشغيل الإلزامية:

```dotenv
RESEND_API_KEY=re_xxx
EMAIL_FROM=Zayed Academy <backup@your-verified-domain.com>
BLOB_READ_WRITE_TOKEN=vercel_blob_rw_xxx
```

الإرسال إلى `enaad4786@gmail.com` لن ينجح باستخدام `onboarding@resend.dev` إلا إذا كان هذا البريد هو بريد حساب Resend نفسه؛ للاستخدام الحقيقي يجب توثيق نطاق تملكه في Resend.
