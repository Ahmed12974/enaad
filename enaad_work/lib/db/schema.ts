...
export const educationalSections = pgTable(
  ...
  (table) => [
    index('educational_sections_status_order_idx').on(table.status, table.sortOrder),
...
export const competitions = pgTable(
  ...
  (table) => [
    index('competitions_active_dates_idx').on(table.isActive, table.startsAt, table.endsAt),
    check(
      'competitions_question_reward_check',
      sql`${table.questionCount} between 1 and 30 and ${table.xpReward} >= 0`,
    ),
    check(
